...
SUCURSALES_CP_PATH = Path(r'C:/Users/Ari/Documents/Dimex-App/Sucursales_codigo_postal.xlsx')
...
def render_sucursal_map_simple(sucursal: str, cp_df: pd.DataFrame):
    ...
    fig.add_trace(go.Scattergeo(
        lat=fila["lat"],
        lon=fila["lon"],
        text=[f"{sucursal} (CP {fila.iloc[0]['CP']})"],
        mode="markers+text",
        marker=dict(size=14, color="#047857", line=dict(color="white", width=1.4)),
    ...
    fig.update_layout(margin=dict(l=0, r=0, t=0, b=0), height=480, showlegend=False)
...
def riesgo_promedio_cluster(df):
    ...
    # ORDENAR por riesgo promedio (ascendente)
    df_plot = df_plot.sort_values("RiesgoPromedio", ascending=True)

    # Colores DIMEX por cluster
    colores = {
        "Cluster_Estrella": "#34D399",   # Verde claro
        "Cluster_Desarrollo": "#059669", # Verde medio
        "Cluster_Riesgo": "#047857"      # Verde oscuro
    }
...
def concent_riesgo_cluster(df):
    ...
    df = df[df[COL_CLUSTER].isin(sel_clusters)]
    if df.empty:
        st.info("No hay datos para los clusters seleccionados.")
        return
...
def main():
    ...
    st.caption(
        f"Fuente de datos: {DATA_PATH.name} · Última actualización: {last_update_txt} · "
